...
describe("DevelopmentScenarioPanel", () => {
  ...
  it("keeps progression configuration schema-valid while disabling exact state", async () => {
    const setScenario = vi
      .spyOn(developmentApi, "setScenario")
      .mockImplementation(async (reference, configuration) => ({
        payment_reference: reference,
    ...
    expect(screen.getByLabelText("Payment state")).toBeDisabled();
    fireEvent.click(screen.getByRole("button", { name: "Apply scenario" }));
  ...
  it("moves the current quote deadline through its dedicated control", async () => {
    const payment = {
      payment_reference: paymentReference,
    } as CreatePaymentResponse;
    const setQuoteExpiry = vi
      .spyOn(developmentApi, "setQuoteExpiry")
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Update quote deadline" }),
    );

    await waitFor(() =>
      expect(setQuoteExpiry).toHaveBeenCalledWith(paymentReference, 8),
    );
    expect(
      await screen.findByText("Quote deadline updated."),
    ).toBeInTheDocument();
  });
});
